...
SETS = {
    ...
    "B": dict(
        ...
        head=["", "되돌아온다", "중간", "길게 간다", "파일"],
        rows=[["약하게", "4샷", "3샷", "3샷", "B_역회전.mp4"],
              ["세게", "4샷", "3샷", "3샷", "〃"]],
        w=[260, 175, 175, 175, 335],
    ),
    "C": dict(
...
def page2():
    ...
    p.grid(s)
    p.t("각도를 재지 마세요. 공이 어떻게 나오는지로 나눕니다.", HOT, 32)
    p.t("★ 세기를 갈라 주세요. 실측으로 확인된 것입니다 (아래).", HOT, 32)

    # ★ 세 무리를 '쿠션에 들어가는 각' 그림 하나로 설명한다.
    #   ⚠️ 전에는 여기에 궤적을 그렸는데 거울반사로 대충 그린 것이라 역회전 실제와
    #      달랐다 (사용자 지적 2026-08-11: '역회전으로 치면 그림과 같이 안 갑니다').
    #      틀린 것을 보여주느니 빼는 것이 낫다 — 쿠션 부근만 그린다.
    p.gap(10)
    y0 = p.y
    d = p.d
    bx0, bx1, by = 200, PW - 200, y0 + 300
    d.rectangle([bx0, by, bx1, by + 40], fill="#2b2f36", outline="#8a93a0", width=2)
    import math as _m
    # ⚠️ **들어가는 쪽만** 그린다. 나가는 쪽을 그리면 거울반사처럼 보이는데
    #    역회전은 거울반사가 아니다. 어떻게 나가는지를 재려고 찍는 것이므로
    #    그림이 답을 미리 말해 버리면 안 된다.
    for ang, lab, col, fx in ((16, "되돌아온다", "#5ec07a", 0.16),
                              (38, "중간", "#ffd400", 0.50),
                              (62, "길게 간다", "#5aa9e6", 0.84)):
        px = bx0 + (bx1 - bx0) * fx
        r = _m.radians(ang)
        ln = 230
        ax, ay = px - _m.sin(r) * ln, by - _m.cos(r) * ln     # 들어오는 쪽
        d.line([px, by, px, by - 170], fill="#4a535f", width=2)   # 쿠션과 직각
        d.line([ax, ay, px, by], fill=col, width=7)
        d.ellipse([ax - 14, ay - 14, ax + 14, ay + 14], fill="#f2f4f8", outline="#14161a", width=3)
        d.text((px, by + 62), lab, font=f(28, True), fill=col, anchor="ma")
    p.y = by + 120
    p.t("가는 실선이 쿠션과 직각. 들어가는 각만 그렸습니다 —", DIM, 29)
    p.t("어떻게 나오는지가 이 촬영으로 재려는 것입니다.", DIM, 29)

    p.h("왜 세기를 가르나  ★ 사용자 지적 · 실측 확인", HOT)
    p.t("0806~0808 의 1294접점을 쿠션 순번까지 맞춰 놓고 비교했습니다.", INK, 30)
    p.gap(6)
    p.t("   입사  5~25도   마주 보고     느림 +29.3   빠름 +17.2    -12.1도", DIM, 28)
    p.t("   입사 25~45도                 느림 +16.6   빠름 +15.7     -0.9도", DIM, 28)
    p.t("   입사 45~65도   비스듬히      느림  +8.6   빠름  +9.7     +1.1도", DIM, 28)
    p.gap(10)
    p.t("마주 보고 넣는 각에서 10~12도나 벌어집니다. 세게 치면 회전이", INK, 30)
    p.t("아직 안 물려서 짧게 나옵니다. 비스듬한 각에서는 1도 안팎입니다.", INK, 30)
    p.t("그래서 '되돌아온다' 칸을 4샷씩으로 두껍게 잡았습니다.", DIM, 28)

    p.rule()
    p.h("왜 역회전이 필요한가", HOT)
    p.t("지금 있는 역회전 자료는 7샷 · 20접점뿐입니다.", INK, 30)
    p.t("그래서 33.jpg 처럼 2쿠션값이 큰 배치에서 '—' 가 뜹니다.", INK, 30)
    p.t("순회전은 130샷 385접점이라 충분한데 역회전만 비어 있습니다.", DIM, 28)

    p.rule()
    p.h("찍는 법", HOT)
    p.t("빈쿠션. 공이 멈출 때까지. 몇 쿠션을 돌든 상관없습니다.", INK, 30)
    p.t("세기는 약·강 두 가지만 확실히 갈라 주시면 됩니다 —", INK, 30)
    p.t("실제 속도는 제가 영상에서 읽습니다.", INK, 30)
    p.save("촬영2.png")
...
def page4():
    ...
    p.box(["빈쿠션 · 공 하나 · 파일 D_기준점.mp4",
           "3쿠션 도착점이 화면에 보이게"])
    p.grid(s, sz=30)
    p.t("세기는 평소 파이브앤하프 칠 때의 세기로 일정하게 해 주세요.", HOT, 30)
