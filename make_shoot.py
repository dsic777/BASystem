...
# ══════════════════════════════════════════════ 촬영0
def page0():
    p = Page("오늘 촬영 — 한 장 요약", "2026-08-11 · 90샷")
    p.t("칸에 적힌 수가 그 칸에서 칠 샷 수다. 빈칸(·)은 안 친다.", DIM, 30)
    p.t("시간이 모자라면 D → C 순서로 버린다. A 는 끝까지.", DIM, 30)
    for k in ("A", "C", "D"):
        s = SETS[k]
        p.gap(24)
...
# ══════════════════════════════════════════════ 촬영2 — B (보류)
def page2():
    """★ 사용자 지적 2026-08-11 로 오늘 촬영에서 뺐다. 왜 뺐는지를 남겨 둔다 —
    나중에 다시 설계할 때 같은 실수를 반복하지 않으려고."""
    p = Page("B  역회전 — 오늘은 보류", "안 찍음")
    p.box(["역회전 세트는 오늘 찍지 않습니다.",
           "지금 설계로는 표가 안 나옵니다."], col=RED, fill="#2a1c1c")

    p.h("왜 뺐나  ★ 사용자 지적", HOT)
    p.t("'정상적인 궤적을 찾는 것이 매우 힘든 일입니다.'", INK, 31)
    p.t("'다이의 특성도 너무 많이 타고, 공의 속도가 빠르면 쿠션의", INK, 31)
    p.t("  반발력 때문에 별로 회전이 걸리지도 않습니다.'", INK, 31)
    p.t("'수구와 쿠션의 거리에 따라서도 다 다르게 나타납니다.'", INK, 31)
    p.gap(16)
    p.t("세어 보면 변수가 넷입니다 —", HOT, 32)
    p.t("   다이 특성 · 속도 · 수구쿠션 거리 · 입사각", DIM, 30)
    p.t("20샷으로는 표가 아니라 흩어진 점 스무 개가 나옵니다.", INK, 31)

    p.rule()
    p.h("더 중요한 것 — 쓰는 자리가 하나다", GRN)
    p.t("'우리 표현으로는 접시라고 하는데, 긴각이든 짧은각이든", INK, 31)
    p.t("  역회전을 주고 다이를 먼저 치면 회전이 반대로 먹어서,", INK, 31)
    p.t("  쿠션을 맞고 반대로 다시 되돌아오는 길일 때 사용합니다.'", INK, 31)
    p.gap(16)
    p.t("접시 조건", HOT, 32)
    p.t("   빈쿠션 (다이를 먼저 친다)", INK, 30)
    p.t("   역회전 9시쯤", INK, 30)
    p.t("   중·하 속도    ★ 강하게 치면 회전이 아예 안 먹는다", INK, 30)
    p.gap(16)
    p.t("앱에 필요한 것은 모든 각도의 역회전 반사표가 아니라", INK, 31)
    p.t("**접시 하나**입니다. 재는 대상이 완전히 다릅니다.", INK, 31)

    p.rule()
    p.h("다음", BLU)
    p.t("사용자가 실전 예시를 그림으로 주기로 했습니다.", INK, 31)
    p.t("그것을 보고 다시 설계합니다. 값은 data/measured_bounce.json 의", DIM, 28)
    p.t("★_역회전_접시 에 그대로 적어 두었습니다.", DIM, 28)
    p.save("촬영2.png")
...
def page3():
    ...
    p.rule()
    p.h("여유가 없으면", DIM)
    p.t("D 다음으로 버리세요. A 가 우선입니다.", INK, 32)
    p.save("촬영3.png")
